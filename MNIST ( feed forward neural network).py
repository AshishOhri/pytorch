...
class NeuralNet(nn.Module):
    def __init__(self,input_size,hidden_size,num_classes):
        ...
        self.relu=nn.ReLU() # to perform rel function on first layer (784)
        self.fc2=nn.Linear(hidden_size,num_classes) # convert 500 elements into 10 elements (digits 0 to 9)
    ...
    def forward(self,x): # Forward propogation
        ...
        out=self.fc2(out) # propogating hidden layer to output layer(10)
        return out
...
for epoch in range(num_epochs):
    for i, (images, labels) in enumerate(train_loader):
        ...
        loss=critetion(outputs,labels) #checking for cross entropy loss
